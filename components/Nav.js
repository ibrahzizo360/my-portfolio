...
const Nav = () => {
  ...
  return (
    <nav className='flex flex-col items-center xl:justify-center gap-y-4 fixed h-max
      bottom-0 mt-auto xl:right-[2%] top-0 z-50 w-full xl:w-16 xl:max-w-md xl:h-screen'>
        {/* inner */}
      <div className='flex w-full items-center justify-between 
        xl:flex-col xl:justify-center gap-y-10 px-4 md:px-40 xl:px-0 h-[80px] 
          ...
          return (
            ...
            href={link.path} key={index}>
              ...
              <div className='absolute pr-14 right-0 hidden xl:group-hover:flex'>
                ...
                border-y-transparent border-y-[6px] border-r-0 absolute right-12 top-[6px]'></div>
              </div>
              {/* icon */} 
             <div>{link.icon}</div>
            </Link>
